...
public final class World extends AbstractHeartBeat {
	...
	private Map<String, Player> __inspectors = Server.getInstance().getPlayerApi().gets();
		...
		if (__sendingInterval >= SEND_PACKETS_RATE) {
			...
			try {
				__workersPool.execute(() -> {
					Map<String, Player> inspectors = new HashMap<String, Player>();
					Iterator<Player> iterators = __inspectors.values().iterator();
					...
					for (int i = 0; i < vehicles.size(); ++i) {
						...
						if (__sendingBroadcast) {
							...
							Iterator<Player> iterator = inspectors.values().iterator();
